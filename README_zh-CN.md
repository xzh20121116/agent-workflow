...
这是硬规则，不是建议。如果你发现编排器在做"绝不"列的事，那就是 bug。
...
| **实现者四状态返回** | `DONE` / `DONE_WITH_CONCERNS` / `NEEDS_CONTEXT` / `BLOCKED` — 编排器处理每种状态 |
