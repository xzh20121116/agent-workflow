...
Agent Workflow 来自**真实的日常使用** — 反复遇到同样的痛点：AI 代理不理解需求就开始乱写代码，主线程上下文膨胀导致目标漂移，前端页面一看就是 AI 做的。
...
| **UI/前端** | **设计约束 + UI 审查员 + AI Slop Score** | -- | -- |
| **审查** | **三阶段**（规格 + 质量 + UI） | 两阶段 | 两阶段 |
| **实现者** | **四状态返回** | 子代理驱动 | 计划驱动 |
| **上下文** | **SubagentContextPacket**（隔离） | baseline 上下文 | 计划即初级工程师 |
| **安装** | **零配置** | doctor 脚本 | 逐宿主插件 |
| **最适合** | **前端项目 + 纪律 + 简洁** | 企业级 baseline | TDD 团队 |

### 为什么前端项目选 Agent Workflow

AI 生成的前端有一种标志性的"塑料感" — Inter 字体、紫蓝渐变、三等分布局、重阴影。**其他工具都不管这个。**

Agent Workflow 从两个方向解决：

1. **预防** — 前端实现者的提示词中注入设计约束：排版规则、配色限制、布局模式、动效指南、图标选择、内容规则
2. **检测** — 专用 UI 审查员检查排版、配色、布局、动效、响应式、无障碍，用 **AI Slop Score (0-10)** 打分

### 为什么上下文管理选 Agent Workflow

AI 编程代理处理复杂任务的头号失败模式：**上下文膨胀 → 目标漂移 → 失忆**。

Agent Workflow 用严格的编排器分离 + SubagentContextPacket 解决：

- 编排器永远不读代码、不写代码、不跑测试 — 上下文保持干净
- 每个子代理拿到自包含的任务包 — 对话历史不泄漏
- 每个阶段结束后写检查点到 handoff.md — 上下文重置不丢进度

Aegis 和 Superpowers 在某些场景下允许主线程碰代码。Agent Workflow 执行硬规则：**碰代码的都是子代理。**

### 为什么简单安装选 Agent Workflow

两个 skill。零配置。不需要 doctor 脚本，不需要 activation mode，不需要 host registry，不需要 host 兼容性矩阵。
